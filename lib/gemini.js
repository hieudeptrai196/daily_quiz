...
// Prompt yêu cầu question ≤ 200, option ≤ 70, code ≤ 12 dòng. Đề hơi dài hơn poll vẫn được chấp nhận
// (quiz.js gửi đề đầy đủ trong 1 tin nhắn); giới hạn dưới đây chặn đề dài lê thê, khó đọc.
const LIMITS = { question: 350, option: 120, codeLines: 18 };
...
// Đề chép lại code (đã có ở code block) làm đề dài và rối: có dòng code ≥ 15 ký tự nằm nguyên trong question
export function copiesCode(question, code) {
  const q = question.replace(/\s+/g, ' ');
  return code
    .split('\n')
    .map((l) => l.trim().replace(/\s+/g, ' '))
    .some((l) => l.length >= 15 && q.includes(l));
}
...
export function normalizeAndValidate(raw, { allowCode }) {
  ...
  if (code && copiesCode(question, code)) return { reason: 'chép code vào question' };
...
export function friendlyReason(reason) {
  ...
  if (/chép code/.test(r)) return 'đề chép lại code, quá dài';
...
function mockGenerate(topic, subtopic, language) {
  ...
  const padding = process.env.GEMINI_MOCK_LONG_TOPIC === topic.key ? ' Bối cảnh dài dòng để vượt giới hạn poll.'.repeat(6) : '';
  return {
